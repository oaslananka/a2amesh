...
`redis` uses an externally managed Redis service for the agent directory, polling leases, and append-only trust log. Configure `registry.storage.redis.existingSecret`, keep the connection URL under its `secretKey`, and set a deployment-specific `prefix`. The chart never places the URL in a ConfigMap or values file. Multi-replica Redis deployments require `distributedPollingLeases=true`; the chart renders a Deployment without a registry PVC. Add a narrow `networkPolicy.registry.extraEgress` rule for the external Redis endpoint and use `rediss://` unless the connection is protected by an equivalent private encrypted boundary.

Memory multi-replica deployments require the explicit `allowEphemeralReplicas` acknowledgement. SQLite remains single-replica. Redis provides shared registry state, but the demo runtime still keeps task state in-process. More than one runtime replica, including HPA, is rejected unless `runtime.autoscaling.allowEphemeralReplicas=true` explicitly acknowledges that requests can be routed to pods that do not share task state. The production profile therefore uses one runtime replica.

The production profile remains a hardened single-node SQLite baseline. Redis enables verified shared registry state and registry replica failover, but it does not make the full platform HA because runtime task state and provider execution are still application-owned. Full-platform HA requires a shared runtime task store, idempotent provider execution, and an operator-verified Redis failover and recovery design.
