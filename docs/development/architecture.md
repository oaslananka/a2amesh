...
| `@a2amesh/mcp`                     | Mapping helpers plus the bounded standalone MCP server and stdio command. It may use core/client public APIs and MCP SDK types.                                                                                                                                                                                                                             |
...
| `agent/getAuthenticatedExtendedCard` | Internal Mesh compatibility path for authenticated extended-card lookup. Official v1 requests use `GetExtendedAgentCard`, HTTP+JSON exposes `GET /extendedAgentCard`, and legacy `agent/authenticatedExtendedCard` remains accepted. |
